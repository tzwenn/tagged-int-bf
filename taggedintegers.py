...
class W_SmallInteger(UnboxedValue):
...
class Tape(object):
    ...
    def get(self):
        return unboxInt(self.thetape[self.position])

    def set(self, val):
        self.thetape[self.position] = W_SmallInteger(val)
    def inc(self):
        self.thetape[self.position] = W_SmallInteger(self.get() + 1)
    def dec(self):
        self.thetape[self.position] = W_SmallInteger(self.get() - 1)
